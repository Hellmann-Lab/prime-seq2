...
#if test -f `ls ${project_folder}/00_fastq/*r1.fq.gz`; #CASE1: demultiplexed using deML and including index reads!
if test -n "$(find ${project_folder}/00_fastq/ -maxdepth 1 -name '*r1.fq.gz' -print -quit)"
then 
file_suffix1="r1.fq.gz"
file_suffix2="r2.fq.gz"
...
for i in `ls ${project_folder}/00_fastq/lane1*${file_suffix1} | xargs -n1 basename`; 
do
	...
	#echo ${sample_name}
	sbatch \
--error="${project_folder}/02_trimming2/${sample_name}cutadapt.%J.err" \
--output="${project_folder}/02_trimming2/${sample_name}cutadapt.%J.out" \
--cpus-per-task=$threads \
--mem=$SLURM_RAM \
--wrap "cutadapt -A A{30} -o ${project_folder}/02_trimming2/${sample_name}r1_trimmed.fq.gz -p ${project_folder}/02_trimming2/${sample_name}r2_trimmed.fq.gz ${project_folder}/00_fastq/${r1_file} ${project_folder}/00_fastq/${r2_file} --discard-casava --minimum-length 28:40 -j ${threads}"
done
