...
--mem=$SLURM_RAM \
--wrap "cutadapt -A A{30} -o ${project_folder}/02_trimming/${sample_name}r1_trimmed.fq.gz -p ${project_folder}/02_trimming/${sample_name}r2_trimmed.fq.gz ${project_folder}/00_fastq/${r1_file} ${project_folder}/00_fastq/${r2_file} --discard-casava --minimum-length 28:40 -j ${threads}"
done
