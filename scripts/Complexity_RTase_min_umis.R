...
here::i_am("scripts/Complexity_RTase_min_umis.R")
...
samples <- data.table(
  sample = c("Maxima_42", "Maxima_50", "SS_42", "SS_50"),
  project = c("Maxima_42", "Maxima_50", "Superscript_42", "Superscript_50")
)
ds_level <- 7000000L
replicates <- 1L
...
for (sample_idx in seq_len(nrow(samples))) {
  sample_name <- samples[sample_idx, sample]
  project_name <- samples[sample_idx, project]

  for (replicate_idx in replicates) {
    print(paste0("sample ", project_name, ", rep ", replicate_idx))

    inputBAM <- paste0(
      "/data/share/htp/Felix_genotyping/primeSeq_tests/RTase_test/trimmed/",
    ...
    counts[, `:=`(
      ...
      ds_level2 = factor(
        "7.0e+06",
        levels = c("No downsampling", "7.0e+06", "5.0e+06", "1.0e+06")
      )
    )]
    setcolorder(counts, c("RG", "n", "project", "replicate", "ds_level", "min_umis", "ds_level2"))
...
saveRDS(
  complexity_min_umis,
  here("data/RTase/complexity_RTase_min_umis.rds")
)
