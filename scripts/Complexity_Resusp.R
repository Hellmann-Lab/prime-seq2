...
here::i_am("scripts/Complexity_Resusp.R")
...
for (k in c(250000, 500000, 750000, 1000000, 1250000, 2000000)){
  for (i in samples){
    print(paste0("sample ", i))
    for (j in 1:2){
      print(paste0("rep ", j, " of 10"))
      inputBAM = paste0("/data/share/htp/Felix_genotyping/primeSeq_tests/BBB_Resusp/trimmed/",i,"/",i,".filtered.Aligned.GeneTagged.sorted.bam")
      bccount = paste0("/data/share/htp/Felix_genotyping/primeSeq_tests/BBB_Resusp/trimmed/",i,"/zUMIs_output/",i,"kept_barcodes_binned.txt")
      ...
      reads <- reads2genes_new_ds(featfile = inputBAM,
                                  ...
                                  downsampling = k)
      print(nrow(reads))
      reads <- reads %>% filter(!is.na(GE)) %>% select(-UB) %>% distinct() %>% count(RG) %>% 
        mutate(project=i) %>%
        mutate(replicate=j) %>%
        mutate(ds_level=k)
...
saveRDS(complexity,
        here("data/BBB_resusp/complexity.rds"))