...
DNA_plot_no_ctrl <- dnase_QF %>%
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar",
               width = 0, position = position_nudge(x = 0.18), linewidth = 0.3) +
  stat_summary(fun = mean, geom = "point",
               position = position_nudge(x = 0.18), shape = 23, size = 2, stroke = 0.6, fill = "white")+
...
complexity <- readRDS(here("data/RTase/complexity_RTase.rds")) %>%
  ...
  mutate(temp_enz_cat = paste0(enzyme, "\n", temp))
...
RT_complexity_plot <- ggplot(complexity, 
  ...
  stat_compare_means(method = "t.test", 
                     paired = FALSE, 
                     label = "p.signif",
                     comparisons = list(c("Superscript IV\n50 °C", "Superscript IV\n42 °C"),
                                        c("Maxima H-\n50 °C", "Maxima H-\n42 °C"),
                                        c("Maxima H-\n50 °C", "Superscript IV\n50 °C"),
                                        c("Maxima H-\n42 °C", "Superscript IV\n42 °C")
                     ),
...
RT_fract_plot <- ggplot(data=map_fract_RT, aes(y=fraction, x=temp_enz_cat, color=temp))+
  geom_quasirandom(alpha=0.5)+
  stat_summary(fun.data = mean_se, geom = "errorbar",
               width = 0) +
  stat_summary(fun = mean, geom = "point",
  ...
  theme(axis.text.x = element_blank(),
    #axis.text.x = element_text(size=13,angle = 90, vjust = 1, hjust = 1),
        strip.text.x = element_text(size=13))+ 
  coord_cartesian(ylim = c(0.8, 0.99))
...
rpc_poolsize <- map_df(seq_along(file_paths9), function(i) {
  ...
  mutate(project = as.numeric(gsub("ng", "", project))) %>%
...
# plot
mapping_fract_poolsize <- ggplot(data=rpc_poolsize, aes(y=fraction, x=project, color=project, group = project))+
  geom_boxplot(aes(color = factor(project)), width = 200)+
  stat_summary(aes(color = factor(project)), fun.data = mean_se, geom = "errorbar",
               width = 0, position = position_nudge(x = 50), linewidth = 0.3) +
  stat_summary(aes(color = factor(project)), fun = mean, geom = "point",
               position = position_nudge(x = 50), shape = 23, size = 2, stroke = 0.6, fill = "white")+
  # stat_compare_means(method = "t.test", 
  #                    paired = FALSE, 
  #                    label = "p.signif",
  ...
  #                                       c("920ng", "80ng"))
  # ) +
  labs(y = "Exon + intron fraction", x = "total RNA input [ng]", color = "total RNA input") +
  theme_pubr(legend = "none") +
  scale_x_continuous(
    breaks = c(80, 320, 920),
    labels = c("80", "320", "920")
...
# Paired t-test: mirrored vs original
mirrored_rows <- lib_data_lig %>% filter(PS == "mirrored")
t_test_mirrored <- t.test(mirrored_rows$readno, mirrored_rows$ctrl_readno_PS, paired = TRUE)
t_test_mirrored

# Paired t-test: blocked vs original
blocked_rows <- lib_data_lig %>% filter(PS == "blocked")
t_test_blocked <- t.test(blocked_rows$readno, blocked_rows$ctrl_readno_PS, paired = TRUE)
t_test_blocked
# Add t-test results to the read_plot_lig plot as annotations

# Extract the p-values from the t-tests
mirrored_pval <- signif(t_test_mirrored$p.value, digits=2)
blocked_pval <- signif(t_test_blocked$p.value, digits=2)
# plot
...
read_plot_lig <- read_plot_lig + 
  annotate("text", 
           x = 1, y = 1.85, 
           label = ifelse(mirrored_pval >= 0.05, "ns", paste0("p = ", mirrored_pval)),
           hjust = 0.5, size = 3.5) +
  annotate("text", 
           x = 2, y = 1.85, 
           label = ifelse(blocked_pval >= 0.05, "ns", paste0("p = ", blocked_pval)),
           hjust = 0.5, size = 3.5)



# Annotate p-values onto the plot (write 'ns' if not significant)
read_plot_lig <- read_plot_lig + 
  annotate("text", 
           x = 1, y = 1.85, 
           label = ifelse(mirrored_pval >= 0.05, "ns", paste0("p = ", mirrored_pval)),
...
unu_nnn <- ggplot(data=unuBC_norm_nnn %>% filter(true_BC==TRUE & PS == "original"), 
  ...
  stat_summary(fun="mean",
               geom="crossbar")+
  coord_cartesian(ylim = c(0.9980954, 1.0070))+
...
plot_fract <- ggplot(data = fractions %>% filter(PS == "original"), aes(y = fraction_norm, x = seq_adapters)) +
  ...
  stat_summary(aes(color=seq_adapters), fun.data = mean_se, geom = "errorbar",
               width = 0, position = position_nudge(x = 0.18), linewidth = 0.3) +
  stat_summary(aes(color=seq_adapters), fun = mean, geom = "point",
               position = position_nudge(x = 0.18), shape = 23, size = 2, stroke = 0.6, fill = "white")+
  geom_hline(yintercept = 1, linetype = "dashed") +
  stat_compare_means(method = "t.test", paired = TRUE, label = "p.signif",
                     comparisons = list(c("P7-RS & P5+RS", "P7-RS & P5-RS"), 
...
BC_PTO_plot <- ggplot() +
  ...
  stat_summary(data = barcode_PTO, aes(y=fraction, x=project, color=project),fun.data = mean_se, geom = "errorbar",
               width = 0, position = position_nudge(x = 0.18), linewidth = 0.3) +
  stat_summary(data = barcode_PTO, aes(y=fraction, x=project, color=project), fun = mean, geom = "point",
               position = position_nudge(x = 0.18), shape = 23, size = 2, stroke = 0.6, fill = "white")+
  geom_text(data = barcode_PTO_avg, aes(y=mean_fraction, x=project, label=mean_fraction))+
  theme_pubr(legend = "none")+
  stat_compare_means(data = barcode_PTO, aes(y=fraction, x=project, color=project), method = "t.test", paired = FALSE, label = "p.signif",
                     ref.group = "improved \n w/o PTO"
  ) +
...
# Make composite
legend4 <- as_ggplot(ggpubr::get_legend(DNA_plot))
layout4 <- "AB\nCC"
...
# plot
read_plot_P5 <- ggplot(data=lib_data_P5, aes(x=PS, y=norm_readno_P5, color=PS))+
  geom_quasirandom(alpha=0.5)+
  stat_summary(fun.data = mean_se, geom = "errorbar",
               width = 0) +
  stat_summary(fun = mean, geom = "point",
...
# P7
read_plot_P7_colored <- ggplot(data=lib_data_P7, aes(x=PS, y=norm_readno_P7, color=PS))+
  geom_quasirandom(alpha=0.5)+
  stat_summary(fun.data = mean_se, geom = "errorbar",
               width = 0) +
  stat_summary(fun = mean, geom = "point",
...
resusp_complexity_plot <- ggplot(data = complexity, 
  ...
  stat_summary(fun = mean, geom = "point",
               shape = 23, size = 2, stroke = 0.6, fill = "white")+
...
### Composite
```{r}
legend6 <- as_ggplot(ggpubr::get_legend(resusp_mapfract_plot))
...
# get fractions
read.delim(here("scripts/antisense_reads/counts_antisense_intergenics_downsampled_poolsize.txt")) %>%
  dplyr::rename(Anti_sense_reads=Count) %>%
  full_join(
    intergenics,
    by=c("Condition"="project")) %>%
  ...
  pivot_longer(cols = 5:6, names_to = "category", values_to = "fraction") %>%
  mutate(Condition = factor(Condition, levels=c("80ng", "320ng", "920ng"))) %>% 
  mutate(fraction_antisense_of_all_intergenic=Anti_sense_reads/Intergenic_reads*100) %>% 
  filter(category=="Intergenic: anti-sense") %>% 
  pull(fraction_antisense_of_all_intergenic)
```
...
## supplement - read funnels alternative samples

```{r}
source("/data/share/htp/prime-seq_NextGen/scripts/read_funnel_alt_samples.R")

read_funnel_all_Liver
read_funnel_all_KOLF

read_funnel_avg_all_Liver
read_funnel_avg_all_KOLF

# plot overview Liver
funnel_plot_Liver <- ggplot()+
  stat_summary(data=read_funnel_all_Liver, 
               aes(y=step, x=reads_norm, color=project), 
               fun.data = "mean_se",
               fun.args = list(mult = 1),
               geom="errorbar", 
               ...
               width=.2,
               size=.5) + 
  geom_line(data=read_funnel_avg_all_Liver, aes(y=step, x=reads_rel, color=project, group=project), size=0.9) + 
  scale_x_continuous(position = "top") + 
  ylab("Processing Steps \n \u27F5") +
  xlab("Liver lysate - Relative Reads")+
  scale_y_discrete(limits = rev(levels(read_funnel_all_Liver$step)))+
  labs(color="Protocol")+
  theme_pubr(legend = "bottom")+
  scale_color_manual(values = color_vector)+
  theme(axis.text.y = element_text(size=13),
...
# plot overview KOLF
funnel_plot_KOLF <- ggplot()+
  stat_summary(data=read_funnel_all_KOLF, 
               aes(y=step, x=reads_norm, color=project), 
               fun.data = "mean_se",
               fun.args = list(mult = 1),
               geom="errorbar", 
               ...
               width=.2,
               size=.5) + 
  geom_line(data=read_funnel_avg_all_KOLF, aes(y=step, x=reads_rel, color=project, group=project), size=0.9) + 
  scale_x_continuous(position = "top") + 
  # ylab("Processing Steps \n \u27F5") +
  xlab("KOLF cell line - Relative Reads")+
  scale_y_discrete(limits = rev(levels(read_funnel_all_KOLF$step)))+
  labs(color="Protocol")+
  theme_pubr(legend = "none")+
  scale_color_manual(values = color_vector)+
  theme(#axis.text.y = element_text(size=13),
...
# composite
alt_samples_suppl <- funnel_plot_Liver + funnel_plot_KOLF + 
  plot_layout(widths = c(1, 1))+
  plot_annotation(tag_levels = 'A')
...
#save
ggsave(alt_samples_suppl,
       filename="/data/share/htp/prime-seq_NextGen/figures/fig_S13_alt_samples_funnels.png",
       height=5,
       width=10
)
