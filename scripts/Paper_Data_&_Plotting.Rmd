...
t_test_P5 <- t.test(lib_data_P5$readno, lib_data_P5$ctrl_readno_P5, paired = TRUE)
...
read_plot_P5 <- read_plot_P5 + 
  annotate("text", 
           x = 1, y = 1.02, 
           label = case_when(t_test_P5$p.value > 0.05 ~ "ns",
                             t_test_P5$p.value <= 0.05 ~ "*",
                             t_test_P5$p.value <= 0.01 ~ "**",
                             t_test_P5$p.value <= 0.001 ~ "***",
                             t_test_P5$p.value <= 0.0001 ~ "****"),
           hjust = 0.5, size = 5)

read_plot_P5

# P7
t_test_P7 <- t.test(lib_data_P7$readno, lib_data_P7$ctrl_readno_P7, paired = TRUE)
...
read_plot_P7_colored <- read_plot_P7_colored + 
  annotate("text", 
           x = 1, y = 2.9, 
           label = case_when(t_test_P7$p.value > 0.05 ~ "ns",
                             t_test_P7$p.value <= 0.05 ~ "*",
                             t_test_P7$p.value <= 0.01 ~ "**",
                             t_test_P7$p.value <= 0.001 ~ "***",
                             t_test_P7$p.value <= 0.0001 ~ "****"),
           hjust = 0.5, size = 5)
