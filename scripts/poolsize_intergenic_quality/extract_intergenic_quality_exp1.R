#!/usr/bin/env Rscript
# Extract quality metrics for intergenic reads from experiment 1
...
here::i_am("scripts/poolsize_intergenic_quality/extract_intergenic_quality_exp1.R")
...
extract_intergenic_quality <- function(bam_file, barcode_file, poolsize) {
  cat("Processing poolsize:", poolsize, "\n")
  ...
  result <- reads_df %>%
    select(poolsize, mapq, aligned_length, soft_clip_fraction, mismatch_rate)
...
# Experiment 1: FC2024_08_01_poolsize
base_dir <- here("data/FC2024_08_01_poolsize/03_zUMIs")
pool_sizes <- c("80ng", "320ng", "920ng")

all_data <- data.frame()

for (ps in pool_sizes) {
  bam_file <- file.path(base_dir, ps, 
                        paste0("poolsize_", ps, ".filtered.Aligned.GeneTagged.sorted.bam"))
  barcode_file <- file.path(base_dir, paste0("BCs_", ps, ".txt"))
  ...
  data <- extract_intergenic_quality(bam_file, barcode_file, ps)
  if (!is.null(data)) {
...
# Save results
output_file <- "intergenic_quality_exp1.rds"
